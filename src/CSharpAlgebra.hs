...
data CSharpAlgebra c m s e
  = CSharpAlgebra
      ...
      , exprConstInt  :: Int -> e
      , exprConstChar :: Char -> e
      , exprConstBool :: Bool -> e
      , exprVar       :: String -> e
...
foldCSharp CSharpAlgebra{..} = fClas
  where
    ...
    fExpr (ExprConstInt  con)   = exprConstInt con
    fExpr (ExprConstChar  con)  = exprConstChar con
    fExpr (ExprConstBool  con)  = exprConstBool con
    fExpr (ExprVar    var)      = exprVar var
    fExpr (ExprOper   op e1 e2) = exprOper op (fExpr e1) (fExpr e2)
    fExpr (ExprCall   call es)  = exprCall call (map fExpr es)
